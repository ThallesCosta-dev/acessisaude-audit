...
def _page_row(
    ...
    return {
        ...
        # Vazio, e não zero: a mesma regra que vale para `lcp_ms`. Sem página
        # auditada não há veredito, e escrever 0 — ou 100 — inventaria um dado.
        "observado": int(score.observed),
        "indice_conformidade": score.conformance_index if score.observed else "",
        "indice_atrito": score.friction_index if score.observed else "",
        "indice_exposicao_juridica": score.legal_exposure_index if score.observed else "",
        "barreira_absoluta": int(score.absolute_barrier) if score.observed else "",
